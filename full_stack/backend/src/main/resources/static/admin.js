...
function renderCharts(dashboard) {
    ...
    const trendLabels = dashboard.trendLabels || [];
    const arrivalTrend = dashboard.arrivalTrend || [];
    const servedTrend = dashboard.servedTrend || [];
    const serviceLabels = (dashboard.serviceQueues || []).map((lane) => formatService(lane.serviceType));
    const serviceLoads = (dashboard.serviceQueues || []).map((lane) => lane.waitingCount);
    ...
    if (!trendChart) {
        ...
        trendChart.data.datasets[0].data = arrivalTrend;
        trendChart.data.datasets[1].data = servedTrend;
        trendChart.options = buildChartOptions(textColor, gridColor);
    ...
    if (!serviceChart) {
        ...
        serviceChart.data.datasets[0].data = serviceLoads;
