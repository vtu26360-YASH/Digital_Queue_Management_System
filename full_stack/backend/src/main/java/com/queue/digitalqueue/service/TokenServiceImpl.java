...
import com.queue.digitalqueue.exception.QueueLimitReachedException;
...
public class TokenServiceImpl implements TokenService {
    ...
    public TokenResponse createToken(CreateTokenRequest request) {
        ...
        if (waitingCount >= maxQueueSize) {
            throw new QueueLimitReachedException("Queue Full. Maximum " + maxQueueSize + " active tokens allowed.");
        }
    ...
    @Override
    @Transactional(readOnly = true)
    public SearchTokenResponse searchTokens(String query) {
        if (query == null || query.isBlank()) {
            return new SearchTokenResponse("", List.of());
        }

        List<Token> waitingTokens = sortWaitingTokens(tokenRepository.findByStatusOrderByCreatedTimeAsc(TokenStatus.WAITING));
        String trimmed = query.trim();
        Integer tokenNumber = parseTokenNumber(trimmed);

        List<TokenResponse> matches = tokenNumber != null
                ? tokenRepository.findByTokenNumber(tokenNumber)
                        .map(token -> List.of(toTokenResponse(token, waitingTokensForService(waitingTokens, token.getServiceType()))))
                        .orElse(List.of())
                : tokenRepository.findTopByNameIgnoreCaseOrderByCreatedTimeDesc(trimmed)
                        .map(token -> List.of(toTokenResponse(token, waitingTokensForService(waitingTokens, token.getServiceType()))))
                        .orElse(List.of());

        return new SearchTokenResponse(trimmed, matches);
    }

    @Override
    @Transactional(readOnly = true)
    public List<TokenResponse> getTokenHistory() {
        List<Token> waitingTokens = sortWaitingTokens(tokenRepository.findByStatusOrderByCreatedTimeAsc(TokenStatus.WAITING));

        return tokenRepository.findByStatusOrderByServedTimeDesc(TokenStatus.SERVED)
                .stream()
                .limit(10)
                .map(token -> toTokenResponse(token, waitingTokensForService(waitingTokens, token.getServiceType())))
                .toList();
    ...
    private Integer parseTokenNumber(String query) {
        try {
            return Integer.parseInt(query.replace("T-", "").trim());
